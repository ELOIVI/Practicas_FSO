...
###############################################################################
usage() {
    echo "Ús: $0 [-r] [-s <similitud>] [-e <ext1,ext2,...>] [-d <subdir>] [-p] <directori1> <directori2>" >&2
...
comparar_permissos() {
    ...
    if [ "$permis1" != "$permis2" ]; then 
        echo "Permisos diferents per $fitxer1 i $fitxer2:"
        echo "$fitxer1: $permis1"
        echo "$fitxer2: $permis2"
    fi
}
...
###############################################################################
echo ""
echo "Comparació de similitud entre tots els fitxers (>= $SIMILARITY_THRESHOLD%):"
